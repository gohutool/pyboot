...
$.v3browser.model = {
    ...
    loadLocalConfig: function(){
		...
		$.app.getJson(API_URL+'/config/getall', null, function(c){			
			CONFIG = c.data
    ...
    saveLocalConfig : function (){
		...
		// $.app.postJson(API_URL+'/config/saveall', CONFIG, function(c){
		// 	// CONFIG = c.data
		// 	// // CONFIG = $.extends.json.toobject($.app.localStorage.getItem(NODES_KEY, "{}"))
    ...
    addNode2Local: function(node){
        ...
        if(idx >= 0){
			...
			$.app.postJson(API_URL+'/proxy/api', node, function(c){
				// CONFIG = c.data
				// // CONFIG = $.extends.json.toobject($.app.localStorage.getItem(NODES_KEY, "{}"))
				...
				// 	CONFIG.nodes = []
				// }
				$.app.show(node.node_name +　'连接信息保存成功');
				
				CONFIG.nodes.push(node);
				$.v3browser.model.saveLocalConfig()
			},null, null,{
				'x-target-url':API_URL+'/config/saveone'
			})
    ...
    removeNode2Local: function(id){
		...
		$.app.postJson(API_URL+'/config/remove/'+id, null, function(c){
			// CONFIG = c.data
			// // CONFIG = $.extends.json.toobject($.app.localStorage.getItem(NODES_KEY, "{}"))
    ...
    removeGroupFromLocal: function(etcdID, groupId, parentRow){
        ...
        if(groups){
			...
			$.app.postJson(API_URL+'/group/saveone/'+node.id, {
				'group': node.group
			}, function(c){
    ...
    saveSearch2Node: function(etcdID, search){
		...
		$.app.postJson(API_URL+'/search/saveone/'+data.node_id, data, function(c){			
			...
			// $.app.show(data.search_name +　'查询信息保存成功');
    ...
    removeSearchFromLocal: function(etcdID, searchId){
		...
		if(idx >=0){
			...
			$.app.postJson(API_URL+'/search/remove/'+one.node_id+'/'+one.id, null, function(c){				
				$.app.show(one.search_name +　'查询信息删除成功');
